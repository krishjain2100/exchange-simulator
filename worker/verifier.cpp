...
private:
  ...
  void HandleCancel(uint64_t target_id, uint16_t inst) {
    ...
    if (loc_it == order_index.end()) return;
    ...
    if (side == Side::BUY) {
      ...
      if (bids[inst][price].empty()) bids[inst].erase(price);
    } else {
      asks[inst][price].erase(list_it);
      if (asks[inst][price].empty()) asks[inst].erase(price);
    }
  ...
  bool WouldSelfMatch(const Order &incoming) {
    ...
    if (incoming.side == Side::BUY) {
      for (const auto &pm : asks[inst]) {
        if (incoming.type == OrderType::LIMIT && incoming.price < pm.first) break;
        if (qty_to_check == 0) break;
        for (const auto &resting : pm.second) {
          if (resting.client_id == client) return true;
          qty_to_check = (qty_to_check > resting.quantity) ? (qty_to_check - resting.quantity) : 0;
          if (qty_to_check == 0) break;
        }
      }
    } else {
...
public:
  ...
  void ProcessOrder(Order current) {
    ...
    if (inst < 1 || inst > NUM_INSTRUMENTS) return;

    if (!seen_sequences.insert(current.sequence_id).second) return;
    ...
    if (WouldSelfMatch(current)) return;
    ...
    if (current.side == Side::BUY) {
      ...
      while (it != ask_book.end() && current.quantity > 0) {
        ...
        while (order_it != orders_at_price.end() && current.quantity > 0) {
          ...
          trades[inst].push_back({inst, maker.sequence_id, current.sequence_id, match_qty, ask_price});
      ...
      if (current.quantity > 0 && current.type == OrderType::LIMIT) {
        ...
        order_index[new_it->sequence_id] = {new_it->price, Side::BUY, new_it};
      }
    } else {
      ...
      while (it != bid_book.end() && current.quantity > 0) {
        ...
        while (order_it != orders_at_price.end() && current.quantity > 0) {
          ...
          // Push to the specific instrument's ledger
          trades[inst].push_back({inst, maker.sequence_id, current.sequence_id, match_qty, bid_price});
      ...
      if (current.quantity > 0 && current.type == OrderType::LIMIT) {
        ...
        lst.emplace_back(current);
        auto new_it = std::prev(lst.end());
        order_index[new_it->sequence_id] = {new_it->price, Side::SELL, new_it};
      }
    }
  }
  ...
  std::vector<Order> GetRestingOrders(uint16_t inst) const {
    ...
    for (const auto &[price, level] : bids[inst]) {
      for (const auto &o : level) resting.push_back(o);
    }
...
int main(int argc, char *argv[]) {
  ...
  if (f_payload) {
      for (int i = 1; i <= NUM_INSTRUMENTS; ++i) {
          ...
          if (fread(&chunk_size, sizeof(uint64_t), 1, f_payload) != 1) break; 
  ...
  std::cout << "[Verifier] Processing " << inputs.size() << " orders through Golden Model...\n";
  ...
  for (int i = 1; i <= NUM_INSTRUMENTS; ++i) {
      ...
      if (cpp_payload[i].empty()) {
          std::cerr << "  FAILED — Engine produced no output payload for Instrument " << i << ".\n";
          return 1;
      }
      ...
      if (golden.trades[i].size() != cpp_payload[i].size()) {
        std::cerr << "  FAILED — Trade count mismatch on Instrument " << i << ": C++ Engine produced "
                  << cpp_payload[i].size() << " trades, Golden Model expects "
                  << golden.trades[i].size() << "\n";
        return 1;
      }

      for (size_t j = 0; j < golden.trades[i].size(); ++j) {
        const auto& t = golden.trades[i][j];
        uint64_t expected_hash = HashTrade(t.inst_id, t.maker_id, t.taker_id, t.qty, t.price);
        
        if (expected_hash != cpp_payload[i][j]) {
            std::cerr << "\n  FAILED — Trade Hash Collision at Index " << j << " on Instrument " << i << "!\n"
      ...
      // We now query the Oracle's resting orders exactly how the Wrapper queried the participant
      ...
      if (expected_book_hash != cpp_book_hash) {
          std::cerr << "  FAILED — Final Book Hash Mismatch on Instrument " << i << ".\n"
                    ...
                    << "  Your engine has a memory leak, a ghost level, or dropped an order.\n";
          return 1;
      }
  }

  std::cout << "\nSUCCESS — C++ Engine is Cryptographically Verified Across All Shards!\n";
  return 0;
}
